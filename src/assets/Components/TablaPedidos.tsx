            ...
            <div className="hidden sm:block">
              <table className="min-w-full table-auto text-sm">
                ...
                <tbody className="bg-white divide-y">
                  {pedidos.map((pedido) => {
                    const badge = ['inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold border', CONFIG_ESTADO[pedido.estado].clase].join(' ')
                    const dot = ['w-2 h-2 rounded-full', CONFIG_ESTADO[pedido.estado].dot].join(' ')
